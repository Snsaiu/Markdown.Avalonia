...
internal class UnitTestConverter
{
    [Test]
    public void TryConvert()
    {
        ...
        foreach (var def in HighlightingManager.Instance.HighlightingDefinitions)
        {
            ...
            foreach (var foreCol in colors)
            {
                ...
                foreach (var brsh in wrapper.NamedHighlightingColors) brsh.Foreground?.GetColor(null);
                ...
                foreach (var rule in cvt.Values) Look(rule);
            }
        }
    }
    ...
    private void Look(HighlightingRuleSet ruleSet)
    {
        ...
        foreach (var rule in ruleSet.Rules) rule.Color?.Foreground?.GetColor(null);
    }
}