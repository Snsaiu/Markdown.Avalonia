...
internal class AutoScaleColumnDefinitions : ColumnDefinitions
{
    ...
    private void _grid_LayoutUpdated(object? sender, EventArgs e)
    {
        ...
        var isEnabled = Controls.AutoScaleColumnDefinitions.GetIsEnabled(_grid);
        if (!isEnabled)
        {
    ...
    {
        var entireWidth = _grid.Bounds.Width;
        var entireColumnWidth = entireWidth / _columnCount;

        var widths = MeasureColumnWidths();

        var scaleIndics = Enumerable.Range(0, _columnCount)
            .Where(i => _baseLength[i].IsPercent)
            .ToArray();

        var fixedIndics = Enumerable.Range(0, _columnCount)
            .Where(i => !_baseLength[i].IsPercent)
            .ToArray();
        ...
        var scaleWidth = scaleIndics.Sum(i => widths[i]);

        var wealthy = entireWidth - scaleWidth;
    ...
    private void CollectWidthFrom(int[] indics, double[] widths, LengthUnit[] units, double colWid)
    {
...
internal readonly struct LengthInfo
{
