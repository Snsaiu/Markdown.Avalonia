...
internal static class SimpleHtmlUtils
{
    ...
    public static int SearchTagRange(string text, Match tagStartPatternMatch)
    {
        ...
        if (tagStartPatternMatch.Value.EndsWith("/>")) return searchStart;
    ...
    {
        var idx = SearchTagRange(text, tagStartPatternMatch);
    ...
    public static int SearchTagEnd(string text, int start, string startTagName)
    {
        ...
        for (;;)
        {
            ...
            if (mch.Value.EndsWith("/>")) continue;
            ...
            if (!string.IsNullOrEmpty(mch.Groups["close"].Value))
            {
                ...
                if (tags.Count == 0) return mch.Index + mch.Length;
            }
            else
            {
                if (s_emptyList.Contains(tags.Peek())) tags.Pop();
