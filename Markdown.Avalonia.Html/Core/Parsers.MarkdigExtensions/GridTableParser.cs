...
public class GridTableParser : IBlockTagParser, IHasPriority
{
    ...
    public bool TryReplace(HtmlNode node, ReplaceManager manager, out IEnumerable<Control> generated)
    {
        ...
        grid.Classes.Add(Markdown.TableClass);

        var border = new Border();
        border.Child = grid;
        border.Classes.Add(Markdown.TableClass);

        if (table.ColumnLengths.All(l => l.Unit != LengthUnit.Percent)) border.Classes.Add("TightTable");
        ...
        ReplaceManager manager)
    {
        ...
        foreach (var rowTag in rows)
        {
            ...
            foreach (var cellTag in rowTag.ChildNodes.CollectTag("td", "th"))
            {
                ...
                var colspan = TryParse(cellTag.Attributes["colspan"]?.Value);
                var rowspan = TryParse(cellTag.Attributes["rowspan"]?.Value);
        ...
        static int TryParse(string? txt)
        {
            return int.TryParse(txt, out var v) ? v : 1;
        }
    }
}