...
public class TypicalParseInfo
{
    public TypicalParseInfo(string[] line)
    {
        ...
        {
            var elementType = AppDomain.CurrentDomain
                .GetAssemblies()
        ...
        if (TagNameReference is not null) TagName = (Tags)Enum.Parse(typeof(Tags), TagNameReference);
    ...
    public void ExtraModifyHyperlink(CHyperlink link, HtmlNode node, ReplaceManager manager)
    {
        ...
        if (href is not null)
        {
            ...
            link.Command = urlTxt =>
            {
                ...
                if (command != null && command.CanExecute(urlTxt)) command.Execute(urlTxt);
            };
        }
    }
