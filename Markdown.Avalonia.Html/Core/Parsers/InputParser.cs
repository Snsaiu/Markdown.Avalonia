    ...
    public bool TryReplace(HtmlNode node, ReplaceManager manager, out IEnumerable<CInline> generated)
    {
        ...
        switch (type)
        {
            ...
            case "range":
                ...
                if (minAttr is not null && double.TryParse(minAttr.Value, out var minVal)) slider.Minimum = minVal;

                var maxAttr = node.Attributes["max"];
                if (maxAttr is not null && double.TryParse(maxAttr.Value, out var maxVal)) slider.Maximum = maxVal;
