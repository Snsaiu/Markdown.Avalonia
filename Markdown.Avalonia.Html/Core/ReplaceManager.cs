...
public class ReplaceManager
{
    ...
    private readonly TextNodeParser textParser;

    public ReplaceManager(SyntaxHighlight highlight, SetupInfo info)
    {
        _inlineBindParsers = new Dictionary<string, List<IInlineTagParser>>();
        _blockBindParsers = new Dictionary<string, List<IBlockTagParser>>();
        _bindParsers = new Dictionary<string, List<ITagParser>>();
    ...
    {
        return _blockBindParsers.ContainsKey(tagName.ToLower());
    }

    public bool MaybeSupportInlineTag(string tagName)
    {
        return _inlineBindParsers.ContainsKey(tagName.ToLower());
    }

    public void Register(ITagParser parser)
    {
        if (parser is IInlineTagParser inlineParser) PrivateRegister(inlineParser, _inlineBindParsers);
        if (parser is IBlockTagParser blockParser) PrivateRegister(blockParser, _blockBindParsers);
        ...
        static void PrivateRegister<T>(T parser, Dictionary<string, List<T>> bindParsers) where T : ITagParser
        {
            foreach (var tag in parser.SupportTag)
            {
                ...
                var parserPriority = GetPriority(parser);
        ...
        {
            return p is IHasPriority prop ? prop.Priority : HasPriority.DefaultPriority;
        }
    }
    ...
    public IEnumerable<StyledElement> ParseChildrenJagigng(IEnumerable<HtmlNode> nodes)
    {
        ...
        if (empNd is null) return ParseJagging(nodes);
    ...
    public IEnumerable<StyledElement> ParseBlockAndInline(HtmlNode node)
    {
        ...
        return UnknownTags switch
        {
            UnknownTagsOption.PassThrough
                => HtmlUtils.IsBlockTag(node.Name) ? new[] { new CTextBlock(new CRun { Text = node.OuterHtml }) } : new[] { new CRun { Text = node.OuterHtml } },
