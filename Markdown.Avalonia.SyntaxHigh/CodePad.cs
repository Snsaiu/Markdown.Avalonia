...
internal class CodePad : Panel
{
    ...
    public Control? ExandableMenu
    {
        ...
        set
        {
            if (_expandableMenu is not null) Children.Remove(_expandableMenu);

            _expandableMenu = value;

            if (_expandableMenu is not null && _isMenuVisible) Children.Add(_expandableMenu);
        }
    }
    ...
    public Control? AlwaysShowMenu
    {
        ...
        set
        {
            if (_alwaysShowMenu is not null) Children.Remove(_alwaysShowMenu);

            _alwaysShowMenu = value;

            if (_alwaysShowMenu is not null) Children.Add(_alwaysShowMenu);
        }
    }
    ...
    protected override void OnPointerExited(PointerEventArgs e)
    {
        ...
        {
            if (Children.Contains(_expandableMenu)) _expandableMenu.IsVisible = false;
    ...
    protected override Size ArrangeOverride(Size finalSize)
    {
        ...
        if (_content is not null) _content.Arrange(new Rect(0, yoffset / 2, finalSize.Width, finalSize.Height));
