...
internal class MixHighlightingBrush : HighlightingBrush
{
    ...
    public override IBrush GetBrush(ITextRunConstructionContext context)
    {
        ...
        return originalBrush is ISolidColorBrush sbrsh ? new SolidColorBrush(WrapColor(sbrsh.Color)) : originalBrush;
    }

    public override Color? GetColor(ITextRunConstructionContext context)
    {
        if (_baseBrush.GetBrush(context) is ISolidColorBrush sbrsh) return WrapColor(sbrsh.Color);
