...
internal abstract class AbstractListParser : BlockParser2
{
    ...
    )
    {
        ...
        var (textMarker, markerPattern, indentAppending)
            = GetTextMarkerStyle(match.Groups["marker"].Value);

        // count indent from first marker with indent
        var countIndent = match.Groups["indent"].Value.Length;
    ...
    {
        if (Regex.IsMatch(markerText, _markerUL_Disc)) return (TextMarkerStyle.Disc, _markerUL_Disc, 2);

        if (Regex.IsMatch(markerText, _markerUL_Box)) return (TextMarkerStyle.Box, _markerUL_Box, 2);

        if (Regex.IsMatch(markerText, _markerUL_Circle)) return (TextMarkerStyle.Circle, _markerUL_Circle, 2);

        if (Regex.IsMatch(markerText, _markerUL_Square)) return (TextMarkerStyle.Square, _markerUL_Square, 2);

        if (Regex.IsMatch(markerText, _markerOL_Number)) return (TextMarkerStyle.Decimal, _markerOL_Number, 3);

        if (Regex.IsMatch(markerText, _markerOL_LetterLower)) return (TextMarkerStyle.LowerLatin, _markerOL_LetterLower, 3);

        if (Regex.IsMatch(markerText, _markerOL_LetterUpper)) return (TextMarkerStyle.UpperLatin, _markerOL_LetterUpper, 3);

        if (Regex.IsMatch(markerText, _markerOL_RomanLower)) return (TextMarkerStyle.LowerRoman, _markerOL_RomanLower, 3);

        if (Regex.IsMatch(markerText, _markerOL_RomanUpper)) return (TextMarkerStyle.UpperRoman, _markerOL_RomanUpper, 3);
