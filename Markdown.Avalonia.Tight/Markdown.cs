...
{
    private readonly string[] _assetAssemblyNames;
...
#pragma warning restore CS0618
    ...
    public IBitmapLoader? BitmapLoader
    {
        ...
        {
            _loader = value;
            if (_loader is not null) _loader.AssetPathRoot = _assetPathRoot;
        }
    }
    ...
    public IContainerBlockHandler? ContainerBlockHandler
    {
        ...
        set => _containerBlockHandler = value;
    }
    ...
    private void SetupParser()
    {
        ...
        if (info.EnableContainerBlockExt) topBlocks.Add(new ContainerBlockParser());
        ...
        if (info.EnableTableBlock) subBlocks.Add(new TableParser());

        if (info.EnableNoteBlock) subBlocks.Add(new NoteParser());
    ...
    private IEnumerable<DocumentElement> PrivateRunBlockGamut(string text, ParseStatus status)
    {
        ...
        for (;;)
        {
            ...
            if (bestBegin > index) RunBlockRest(text, index, bestBegin - index, status, 0, rtn);
        ...
        if (index < text.Length) RunBlockRest(text, index, text.Length - index, status, 0, rtn);
            ...
            List<DocumentElement> outto)
        {
            for (; parserStart < _blockParsers.Length; ++parserStart)
            {
                ...
                for (;;)
                {
                    ...
                    var rslt = parser.Convert2(text, match, status, this, out var parseBegin, out var parserEnd);
                    if (rslt is null) break;

                    if (parseBegin > index) RunBlockRest(text, index, parseBegin - index, status, parserStart + 1, outto);
                    outto.AddRange(rslt);
            ...
            if (length != 0) outto.AddRange(FormParagraphs(text.Substring(index, length), status));
        }
    }
            ...
            List<CInline> outto)
        {
            for (; parserStart < _inlines.Length; ++parserStart)
            {
                ...
                for (;;)
                {
                    ...
                    var rslt = parser.Convert(text, match, this, out var parseBegin, out var parserEnd);
                    if (rslt is null) break;

                    if (parseBegin > index) RunSpanRest(text, index, parseBegin - index, parserStart + 1, outto);
                    outto.AddRange(rslt);
            ...
            if (length != 0)
            {
                ...
                outto.AddRange(
                    StrictBoldItalic ? DoText(subtext) : DoTextDecorations(subtext, s => DoText(s)));
            }
        }
    }
        ...
        where TContent : Control
    {
        ...
        foreach (var c in content) result.Children.Add(c);
    ...
    private IEnumerable<DocumentElement> FormParagraphs(string text, ParseStatus status)
    {
        ...
        var grafs = trimemdText == "" ? new string[0] : _newlinesMultiple.Split(trimemdText);
    ...
    {
        if (string.IsNullOrEmpty(match.Groups[2].Value)) return TreatsAsHref(match);
    ...
    private CInline TreatsAsHref(Match match)
    {
        ...
        var link = new CHyperlink(PrivateRunSpanGamut(linkText))
        {
            Command = urlTxt =>
            {
                if (HyperlinkCommand != null && HyperlinkCommand.CanExecute(urlTxt)) HyperlinkCommand.Execute(urlTxt);
            },
    ...
    {
        if (!string.IsNullOrEmpty(match.Groups[0].Value) && match.Groups[0].Value.StartsWith("$")) return LoadMathExpressionImage(match.Groups[2].Value);

        var altText = match.Groups[3].Value;
        var urlTxt = match.Groups[4].Value;
        var title = match.Groups[7].Value;
    ...
    private CInline LoadImage(string urlTxt, string title)
    {
        ...
        {
            if (resourceVal is Control control) return new CInlineUIContainer(control);

            CImage? cimg = null;
            if (resourceVal is Bitmap renderedImage) cimg = new CImage(renderedImage);
        ...
        var image = _setupInfo.LoadImage(urlTxt);
    ...
    private IEnumerable<CInline> DoTextDecorations(string text, Func<string, IEnumerable<CInline>> defaultHandler)
    {
        ...
        if (buff.Length > 0) rtn.AddRange(defaultHandler(buff.ToString()));
...
internal struct Candidate<T> : IComparable<Candidate<T>>
{
    ...
    {
        return Match.Index.CompareTo(other.Match.Index);
    }
}
