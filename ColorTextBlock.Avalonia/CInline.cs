...
public abstract class CInline : StyledElement
{
    ...
    {
        get => GetValue(IsStrikethroughProperty);
    ...
    protected void RequestMeasure()
    {
        ...
        else if (Parent is Layoutable layout) layout.InvalidateMeasure();
    }
    ...
    protected void RequestRender()
    {
        try
        {
            if (Parent is CInline cline)
                cline.RequestRender();
            else if (Parent is Layoutable layout) layout.InvalidateVisual();
        }
        catch
        {
