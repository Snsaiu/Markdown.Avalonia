...
public class CTextBlock : Control, ITextPointerHandleable
{
    ...
    private readonly List<CGeometry> _intermediates = new();
    ...
    public Selection? Selection =>
        _beginSelect is not null && _endSelect is not null ? new Selection(_beginSelect.Index, _endSelect.Index) : null;
    ...
    public TextPointer CalcuatePointerFrom(int index)
    {
        ...
        foreach (var metry in _metries)
        {
            ...
            if (index < caretLength) return metry.CalcuatePointerFrom(index);
    ...
    {
        if (_metries.Count != 0) return _metries[0].GetBegin().Wrap(this, 0);
    ...
    public TextPointer GetEnd()
    {
        if (_metries.Count != 0)
        {
            ...
            var indexAdd = _metries.Take(_metries.Count - 1).Sum(t => t.CaretLength);
    ...
    protected override Size ArrangeOverride(Size finalSize)
    {
        ...
        if (AreClose(_constraint.Width, finalSize.Width)) return finalSize;
    ...
    private static bool AreClose(double value1, double value2)
    {
        ...
        var tolerance = (Math.Abs(value1) + Math.Abs(value2) + 10d) * 1e-10;
    ...
    private Size UpdateGeometry()
    {
        ...
        if (_beginSelect is not null && _endSelect is not null) Select(_beginSelect.Index, _endSelect.Index);
    ...
    {
        if (Background is not null) context.FillRectangle(Background, new Rect(0, 0, Bounds.Width, Bounds.Height));

        var select = SelectionBrush ?? Brushes.Cyan;
        ...
        if (_beginSelect is not null && _endSelect is not null)
        {
            ...
            else
            {
                ...
                foreach (var inter in _intermediates) TryRender(inter, new Rect(inter.Left, inter.Top, inter.Width, inter.Height));
        ...
        if (fillAfter is not null)
        {
            if (select is ISolidColorBrush colorBrush)
            {
                ...
                foreach (var fillRct in fillAfter) context.FillRectangle(selectFill, fillRct);
            }
            else
            {
    ...
    private void ComplementIntermediate()
    {
        ...
        foreach (var metry in _metries)
        {
            ...
            if (bgn | end) _intermediates.Add(metry);
        }
    }
    ...
    {
        if (_beginSelect is null || _endSelect is null) return string.Empty;
        ...
        if (ReferenceEquals(bgn.Geometry, end.Geometry))
        {
            if (bgn.Geometry is TextLineGeometry tlg) return tlg.Text.Substring(bgn.InternalIndex, end.InternalIndex - bgn.InternalIndex);
