...
public class CRun : CInline
{
    ...
    protected override IEnumerable<CGeometry> MeasureOverride(
        ...
        double remainWidth)
    {
        if (string.IsNullOrEmpty(Text)) return Array.Empty<CGeometry>();
        ...
        if (remainWidth == entireWidth) return CreateLines(source, entireWidth, paraProps);

        var firstLine = TextFormatter.Current.FormatLine(source, 0, double.PositiveInfinity, paraProps);
        if (firstLine is null) return Array.Empty<CGeometry>();

        if (firstLine.Width < remainWidth)
        {
            if (firstLine.Length == Text.Length) return new List<CGeometry> { new TextLineGeometry(this, source, firstLine, false) };
        ...
        var breakPos = breakPosEnum.MoveNext(out var lnbrk) ? lnbrk.PositionWrap : int.MaxValue;
        ...
        CGeometry? prevGeo = null)
    {
        ...
        var prev = TextFormatter.Current.FormatLine(source, 0, entireWidth, paraProps);
        if (prev is null)
            yield break;
    ...
    {
        return new GenericTextRunProperties(Typeface, FontSize, foregroundBrush: foreground);
    }
...
internal readonly struct SimpleTextSource : ITextSource
{
    ...
    public TextRunProperties RunProperties { get; }
    ...
    {
        return new TextCharacters(_text.Slice(textSourceIndex), RunProperties);
    }

    public SimpleTextSource Subsource(int start, int length)
    {
        return new SimpleTextSource(_text.Slice(start, length), RunProperties);
    }

    public string Substring(int start, int length)
    {
        return _text.Slice(start, length).ToString();
    }

    public string Substring(int start)
    {
        return _text.Slice(start).ToString();
    }

    public SimpleTextSource ChangeForeground(IBrush? foreground)
    {
        var runProps = new GenericTextRunProperties(RunProperties.Typeface, RunProperties.FontRenderingEmSize, foregroundBrush: foreground);
