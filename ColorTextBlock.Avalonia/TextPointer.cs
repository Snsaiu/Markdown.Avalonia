...
public class TextPointer : IEquatable<TextPointer>, IComparable<TextPointer>
{
    ...
    {
        return other is not null ? Index.CompareTo(other.Index) : throw new ArgumentNullException(nameof(other));
    }

    public bool Equals(TextPointer? other)
    {
        return PathDepth == other.PathDepth
               && Enumerable.Range(0, PathDepth).All(i => ReferenceEquals(_path[i], other[i]))
               && Index == other.Index
